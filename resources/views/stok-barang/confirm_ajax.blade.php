...
@else
    <form action="{{ url('/stok-barang/' . $stok->stok_id) }}" method="POST" id="form-delete">
        @csrf
        @method('DELETE')
        <div id="modal-master" class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Hapus Data Stok Barang</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    <div class="alert alert-warning">
                        ...
                        Apakah Anda ingin menghapus data stok barang berikut?
                    </div>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status) {
                                ...
                                dataStok.ajax.reload(); // Ganti sesuai ID DataTable kamu
                            } else {
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Gagal',
