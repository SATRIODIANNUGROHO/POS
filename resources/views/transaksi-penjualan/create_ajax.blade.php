<form action="{{ url('/transaksi-penjualan') }}" method="POST" id="form-tambah">
    @csrf
    <div id="modal-master" class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Tambah Data Transaksi Penjualan</h5>
                ...
                <div class="form-group">
                    ...
                    <select name="penjualan_id" id="penjualan_id" class="form-control" required>
                        ...
                        @foreach ($penjualanList as $p)
                            <option value="{{ $p->penjualan_id }}">{{ $p->penjualan_id }}</option>
                        @endforeach
                    </select>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) {
                            ...
                            dataTransaksiPenjualan.ajax.reload(); // ganti sesuai nama DataTable kamu
                        } else {
                            $('.error-text').text('');
